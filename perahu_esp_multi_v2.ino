...
void task_baca_sensor(void* pvParameters) {
  ...
  while (1) {
    ...
    if (
      (rerata[0] < AMBANG_BATAS) &&
      (rerata[1] < AMBANG_BATAS) &&
      (rerata[2] < AMBANG_BATAS) &&
      (rerata[3] < AMBANG_BATAS)
    ) {
      tinggi = TINGKAT_0;
      menguras = 0;
    } else {
    ...
    if (tinggi >= TINGKAT_1 && menguras) {
      digitalWrite(PIN_RELAY, LOW); // Pompa nyala
      is_aktif = 1;
    } else if (tinggi < TINGKAT_1 && !menguras) {
      digitalWrite(PIN_RELAY, HIGH); // Pompa mati
      is_aktif = 0;
    }
    ...
    if (tinggi != tinggi_prev) {
      ...
      // xQueueSend(adcQueue, &tinggi, 0);
      xQueueOverwrite(adcQueue, &tinggi);
    }
...
void task_kirim_data(void* pvParameters) {
  ...
  while (1) {
    ...
    // if (xQueueReceive(adcQueue, &tinggi, 0) != pdTRUE) {
      vTaskDelay(pdMS_TO_TICKS(10));
      continue;
    }
